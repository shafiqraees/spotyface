...
class Requests extends Controller {
    ...
    public function feature_portfolio() {
        $model = $this->model('Requests');
		$input = $this->library('Input');
		
        $id = $input->get('id');
						
		 $delete = $model->feature_portfolio($id);
		
		 if ($delete == 1) {
				$response['status']  = $this->lang['success'];
				$response['message'] = $this->lang['featured_successfully'];
		 } else {
				$response['status']  = $this->lang['error'];
				$response['message'] = $this->lang['unable_to_feature'];
		 }	

        return $response;
    }

    public function un_feature_portfolio() {
        $model = $this->model('Requests');
		$input = $this->library('Input');
		
        $id = $input->get('id');
						
		 $delete = $model->un_feature_portfolio($id);
		
		 if ($delete == 1) {
				$response['status']  = $this->lang['success'];
				$response['message'] = $this->lang['removed_successfully'];
		 } else {
				$response['status']  = $this->lang['error'];
				$response['message'] = $this->lang['unable_to_remove'];
		 }	
