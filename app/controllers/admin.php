...
class Admin extends Controller
{
    ...
    public function portfolios()
    {
		...
		/* Use Admin Model */
		$adminModel = $this->model('Admin');
        $data['portfolios'] = $adminModel->portfolios();
        ...
        if(isset($this->url[2]) && $this->url[2] == 'edit') {			
            ...
            $has = $adminModel->has_portfolio($this->url[3]);
            ...
            if($has === true) {
				...
				/* Category data */
                $data["portfolio"] = $adminModel->get_portfolio($this->url[3]);
							 ...
							 if(move_uploaded_file($_FILES['photoimg']['tmp_name'], $path.$fileName))
							  {
								...
								$update = $adminModel->update_portfolio($input->get('description'), $fileName, $input->get('id'));
									
								if ($update == 1) {
									$_SESSION['message'][] = ['success', $this->lang['details_updated']];
									redirect('admin/portfolios/edit/'. $input->get('id'));
								} else {
									$_SESSION['message'][] = ['warning', $this->lang['no_changes_made']];
									redirect('admin/portfolios/edit/'. $input->get('id'));
								}					  
								...
								$update = $adminModel->update_portfolio($input->get('description'), $input->get('image'), $input->get('id'));
                ...
                return ['content' => $this->view->render($data, 'admin/edit_portfolio')];
        ...
        return ['content' => $this->view->render($data, 'admin/portfolios')];
    }
